...
void resolve_hostname(char *server_host, char *host_to_resolve) {
    ...
    char* ret = check_cache_for_address(host_to_resolve, CACHE_FILE);
    if (ret) {
	print_ipv4_address_in_cache(host_to_resolve, ret);
	exit(0);
    }
    ...
    print_ipv4_address(res->address->host, res->address->host_ipv4_address);
//    free(q);
}
