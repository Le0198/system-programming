...
vector *vector_create(copy_constructor_type copy_constructor,
    ...
    ptr-> size = 0;
    ptr->array = malloc(sizeof(void*) * INITIAL_CAPACITY);
...
void vector_resize(vector *this, size_t n) {
    ...
    if(this->size > n){
	int i;
	for(i = (int)this->size - 1; i >= (int)n; i--){
	    this->destructor(*(this->array + i));
	}
    }
    else if(this->size < n){
...
void vector_insert(vector *this, size_t position, void *element) {
    ...
    this->size++;
    if(this->size > this->capacity){
	this->array = realloc(this->array, get_new_capacity(this->size)*sizeof(void*));
	this->capacity = get_new_capacity(this->size);
    }
    size_t i;
    for(i = this->size - 1; i > position; i--){
	//copy_constructor???
	//
